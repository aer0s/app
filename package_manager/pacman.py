...
class Pacman(ManagerBase):
    ...
    def info(self, *pkgs):
        ...
        self.sudo_pkm(*packages, S=True, i=True, _fg=True)

    def install(self, *pkgs, noconfirm):
        packages = self.parse_pkgs(*pkgs, just_names=True)
        self.sudo_pkm(*packages, S=True, noconfirm=noconfirm, _fg=True)

    def remove(self, *pkgs, noconfirm):
        packages = self.parse_pkgs(*pkgs, just_names=True)
        self.sudo_pkm(*packages, R=True, noconfirm=noconfirm, _fg=True)
    ...
    def update(self, noconfirm):
        self.sudo_pkm(S=True, y=True, noconfirm=noconfirm, _fg=True)

    def upgrade(self, *pkgs, noconfirm):
        packages = self.parse_pkgs(*pkgs, just_names=True)
        self.sudo_pkm(S=True, u=True, *packages, noconfirm=noconfirm, _fg=True)
