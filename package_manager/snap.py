...
class Snap(ManagerBase):
    ...
    def remove(self, *pkgs, noconfirm):
        for pkg in self.parse_pkgs(*pkgs):
            self.sudo_pkm.remove(
                pkg["pkg"], channel=pkg.get("variant", "stable"), _fg=True
            )
